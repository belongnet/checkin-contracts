...
describe.only("NFTV2", () => {
  ...
  let instanceInfoETH: InstanceInfoStruct,
    ...
    royalties: RoyaltiesParametersStruct,
    implementations: ImplementationsStruct;

  async function fixture() {
    const [owner, alice, bob, charlie, pete] = await ethers.getSigners();
    const signer = EthCrypto.createIdentity();

    const Validator: ContractFactory = await ethers.getContractFactory(
      "MockTransferValidator"
    );
    const validator: MockTransferValidator = (await Validator.deploy(
      true
    )) as MockTransferValidator;
    await validator.deployed();

    const Erc20Example: ContractFactory = await ethers.getContractFactory(
      "WETHMock"
    );
    const erc20Example: WETHMock = (await Erc20Example.deploy()) as WETHMock;
    await erc20Example.deployed();

    const NFTImplementation: ContractFactory = await ethers.getContractFactory(
      "NFTV2"
    );
    const nft: NFT = (await NFTImplementation.deploy()) as NFT;
    await nft.deployed();

    const RRImplementation: ContractFactory = await ethers.getContractFactory(
      "RoyaltiesReceiverV2"
    );
    const rr: RoyaltiesReceiver =
      (await RRImplementation.deploy()) as RoyaltiesReceiver;
    await rr.deployed();
    ...
    const NFTFactory: ContractFactory = await ethers.getContractFactory(
      "NFTFactoryV2"
    );
    ...
    const RoyaltiesReceiver = await ethers.getContractFactory(
      "RoyaltiesReceiverV2"
    );
  ...
  describe("Deployment", () => {
    it("Should be deployed correctly", async () => {
      ...
      expect(infoReturned.metadata.name).to.be.equal(
        instanceInfoETH.metadata.name
      );
      expect(infoReturned.metadata.symbol).to.be.equal(
      ...
      expect(infoReturned.transferable).to.be.equal(
        instanceInfoETH.transferable
      );
      ...
      expect((await nft_eth.parameters()).info.whitelistMintPrice).to.be.equal(
        instanceInfoETH.whitelistMintPrice
      );
      expect((await nft_eth.parameters()).info.transferable).to.be.equal(
        instanceInfoETH.transferable
      );
      expect((await nft_eth.parameters()).info.maxTotalSupply).to.be.equal(
        instanceInfoETH.maxTotalSupply
      );
      expect((await nft_eth.parameters()).info.feeNumerator).to.be.equal(
        instanceInfoETH.feeNumerator
      );
      expect((await nft_eth.parameters()).creator).to.be.equal(alice.address);
      expect((await nft_eth.parameters()).info.collectionExpire).to.be.equal(
        instanceInfoETH.collectionExpire
      );
      expect(await nft_eth.contractURI()).to.be.equal(
        instanceInfoETH.contractURI
      );
      expect(await nft_eth.getTransferValidator()).to.be.equal(
        validator.address
      );

      [, , , feeReceiver, , infoReturned] = await nft_erc20.parameters();
      expect(infoReturned.maxTotalSupply).to.be.equal(
        instanceInfoToken.maxTotalSupply
      );
      expect(infoReturned.feeNumerator).to.be.equal(
        instanceInfoToken.feeNumerator
      );
      expect(feeReceiver).to.be.equal(receiver_erc20.address);
      expect(infoReturned.collectionExpire).to.be.equal(
      ...
      expect(await nft_eth.supportsInterface(interfaceIdICreatorToken)).to.be
        .true;
      expect(await nft_eth.supportsInterface(interfaceIdILegacyCreatorToken)).to
        .be.true;

      const [functionSignature, isViewFunction] =
        await nft_eth.getTransferValidationFunction();
  ...
  describe("Functions from AutoValidatorTransferApprove and CreatorToken", () => {
    it("Functions from AutoValidatorTransferApprove", async () => {
      ...
      expect(
        await nft_eth_alice.isApprovedForAll(alice.address, validator.address)
      ).to.be.true;
      expect(
        await nft_eth_alice.isApprovedForAll(validator.address, alice.address)
      ).to.be.false;
  ...
  describe("Mint", () => {
    it("Should mint correctly static prices", async () => {
      const { nft_eth, receiver_eth, owner, alice, signer } = await loadFixture(
        fixture
      );
    ...
    it("Should batch mint correctly dynamic prices", async () => {
      ...
      await nft_eth
        .connect(alice)
        .mintDynamicPrice(alice.address, dynamicParams, ETH_ADDRESS, {
    ...
    it("Should mint correctly with erc20 token", async () => {
      const { nft_erc20, alice, erc20Example, signer } = await loadFixture(
        fixture
      );
      const NFT_721_BASE_URI = "test.com/";
    ...
    it("Should mint correctly with erc20 token without fee", async () => {
      const { factory, nft_erc20, alice, erc20Example, signer, owner } =
        await loadFixture(fixture);
    ...
    it("Should transfer if transferrable", async () => {
      const { factory, nft_erc20, alice, erc20Example, signer, owner, bob } =
        await loadFixture(fixture);
    ...
    it("Should fail with 0 acc balance erc20", async () => {
      const { alice, signer, nft_erc20, erc20Example } = await loadFixture(
        fixture
      );
      ...
      await erc20Example
        .connect(alice)
        .approve(nft_erc20.address, 99999999999999);
  ...
  describe("TokenURI test", async () => {
    it("Should return correct metadataUri after mint", async () => {
      ...
      await expect(nft_eth.tokenURI(100)).to.be.revertedWithCustomError(
        nft_eth,
        "TokenIdDoesNotExist"
      );
  ...
  describe("Withdraw test", async () => {
    it("Should withdraw all funds when contract has 0 comission", async () => {
      const { alice, nft_erc20, erc20Example, signer, factory, owner } =
        await loadFixture(fixture);
    ...
    it("Should withdraw all funds without 10% (comission)", async () => {
      ...
      const fullFees = BigNumber.from(token_price)
        ...
        .div(10000);
      const feesToRefferalCreator = await factory.getReferralRate(
        alice.address,
        hashedCode,
        fullFees
      );
    ...
    it("Should withdraw all funds when contract has 0 comission", async () => {
      const { alice, nft_eth, signer, factory, owner } = await loadFixture(
        fixture
      );
    ...
    it("Should correct distribute royalties 2 payees", async () => {
      ...
      await receiver.initialize(
        {
          creator: alice.address,
          platform: (await factory.nftFactoryParameters()).platformAddress,
      ...
      const platformAddress = (await factory.nftFactoryParameters())
        .platformAddress;
      ...
      await receiver.connect(bob).releaseAll(await receiver.ETH_ADDRESS());

      expect(await receiver.totalReleased(await receiver.ETH_ADDRESS())).to.eq(
        ethers.utils.parseEther("1")
      );
      expect(
        await receiver.released(await receiver.ETH_ADDRESS(), alice.address)
      ).to.eq(ethers.utils.parseEther("0.8"));
      expect(
        await receiver.released(await receiver.ETH_ADDRESS(), owner.address)
      ...
      await receiver.connect(bob).releaseAll(erc20Example.address);

      expect(await receiver.totalReleased(erc20Example.address)).to.eq(
        ethers.utils.parseEther("1")
      );
      expect(
        await receiver.released(erc20Example.address, alice.address)
      ).to.eq(ethers.utils.parseEther("0.8"));
      expect(
        await receiver.released(erc20Example.address, owner.address)
      ).to.eq(ethers.utils.parseEther("0.2"));
      ...
      await receiver
        .connect(bob)
        .release(await receiver.ETH_ADDRESS(), alice.address);
      ...
      await receiver.connect(bob).release(erc20Example.address, owner.address);
      ...
      await expect(
        receiver.connect(bob).release(erc20Example.address, charlie.address)
    ...
    it("Should correct distribute royalties 3 payees", async () => {
      ...
      await receiver_eth
        .connect(pete)
        .releaseAll(await receiver_eth.ETH_ADDRESS());

      expect(
        await receiver_eth.totalReleased(await receiver_eth.ETH_ADDRESS())
      ).to.eq(ethers.utils.parseEther("1"));
      expect(
        await receiver_eth.released(
          await receiver_eth.ETH_ADDRESS(),
          alice.address
        )
      ).to.eq(ethers.utils.parseEther("0.8"));
      expect(
      ...
      ).to.eq(ethers.utils.parseEther("0.14"));
      expect(
        await receiver_eth.released(
          await receiver_eth.ETH_ADDRESS(),
          bob.address
      ...
      await erc20Example
        .connect(owner)
        .mint(receiver_eth.address, ethers.utils.parseEther("1"));

      await receiver_eth.connect(pete).releaseAll(erc20Example.address);

      expect(await receiver_eth.totalReleased(erc20Example.address)).to.eq(
        ethers.utils.parseEther("1")
      );
      expect(
        await receiver_eth.released(erc20Example.address, alice.address)
      ).to.eq(ethers.utils.parseEther("0.8"));
      expect(
        await receiver_eth.released(erc20Example.address, owner.address)
      ).to.eq(ethers.utils.parseEther("0.14"));
      expect(
        await receiver_eth.released(erc20Example.address, bob.address)
      ).to.eq(ethers.utils.parseEther("0.06"));
