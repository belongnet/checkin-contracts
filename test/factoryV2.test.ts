...
describe("NFTFactoryV2", () => {
  ...
  describe("Deployment", () => {
    it("should correct initialize", async () => {
      ...
      expect(
        (await factory.nftFactoryParameters()).platformCommission
      ).to.be.equal(+PLATFORM_COMISSION);
      expect((await factory.nftFactoryParameters()).signerAddress).to.be.equal(
        signer.address
      );
      expect(
        (await factory.nftFactoryParameters()).defaultPaymentCurrency
      ).to.be.equal(ETH_ADDRESS);
      expect((await factory.nftFactoryParameters()).maxArraySize).to.be.equal(
        factoryParams.maxArraySize
      );
      expect(
        (await factory.nftFactoryParameters()).transferValidator
      ).to.be.equal(validator.address);
  ...
  describe("Deploy NFT", () => {
    it("should correct deploy NFT instance", async () => {
      ...
      await expect(
        factory.connect(alice).produce(fakeInfo, ethers.constants.HashZero)
      ...
      const RoyaltiesReceiver: RoyaltiesReceiver = await ethers.getContractAt(
        "RoyaltiesReceiverV2",
        feeReceiver
      );
    ...
    it("should correctly deploy several NFT nfts", async () => {
      const { factory, alice, bob, charlie, signer } = await loadFixture(
        fixture
      );
  ...
  describe("Deploy NFT", () => {
    it("Can create referral code", async () => {
      ...
      await expect(tx)
        .to.emit(factory, "ReferralCodeCreated")
    ...
    it("Can set referral", async () => {
      ...
      await expect(tx)
        .to.emit(factory, "ReferralCodeUsed")
        .withArgs(hashedCode, bob.address);
      expect((await factory.getReferralUsers(hashedCode))[0]).to.eq(
        bob.address
      );

      const amount = 10000;
      await expect(
        factory.getReferralRate(bob.address, hashedCodeFalse, amount)
      )
        .to.be.revertedWithCustomError(factory, "ReferralCodeNotUsedByUser")
        .withArgs(bob.address, hashedCodeFalse);
      expect(
        await factory.getReferralRate(bob.address, hashedCode, amount)
      ).to.eq(amount / 2);
      ...
      expect(
        await factory.getReferralRate(bob.address, hashedCode, amount)
      ).to.eq(getPercentage(amount, referralPercentages[2]));
      ...
      expect(
        await factory.getReferralRate(bob.address, hashedCode, amount)
      ).to.eq(getPercentage(amount, referralPercentages[3]));
      ...
      expect(
        await factory.getReferralRate(bob.address, hashedCode, amount)
      ).to.eq(getPercentage(amount, referralPercentages[4]));
  ...
  describe("Works properly", () => {
    it("Can set params", async () => {
      ...
      await expect(
        factory
          .connect(alice)
          .setFactoryParameters(_factoryParams, referralPercentages)
