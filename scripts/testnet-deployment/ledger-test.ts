...
} from "../../typechain-types/contracts/factories/NFTFactory";
...
const platformCommission = defaultParamsCheck(
  process.env.PLATFORM_COMMISSION,
  200
) as number;
const transferValidator = defaultParamsCheck(
  process.env.TRANSFER_VALIDATOR,
  ZERO_ADDRESS
) as string;
const defaultPaymentCurrency = defaultParamsCheck(
  process.env.PAYMENT_CURRENCY,
  ETH_ADDRESS
) as string;
const maxArraySize = defaultParamsCheck(
  process.env.MAX_ARRAY_SIZE,
  20
) as number;
...
const referralPercentages: number[] = [
  0,
  defaultParamsCheck(
    process.env.REFERRAL_PERCENT_FIRST_TIME_USAGE,
    5000
  ) as number,
  defaultParamsCheck(
    process.env.REFERRAL_PERCENT_SECOND_TIME_USAGE,
    3000
  ) as number,
  defaultParamsCheck(
    process.env.REFERRAL_PERCENT_THIRD_TIME_USAGE,
    1500
  ) as number,
  defaultParamsCheck(
    process.env.REFERRAL_PERCENT_THIRD_TIME_USAGE,
