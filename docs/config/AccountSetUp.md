...
- Starkli is installed correctly. Ensure that the following command shows the version information for Starkli:
  ...
  If this command fails, see [Setting up your environment](./EnvSetUp.md).
...
For testnet transactions, you can fund your wallet using one of the [Starknet Sepolia faucets](https://www.starknet.io/en/ecosystem/bridges-and-onramps).
...
> **NOTE**
> If you are working with a Braavos wallet, the type is defined as `braavos` and the account descriptor structure might be slightly different.
...
> **NOTE**
> This command requires a signer. If you receive an error after running this command, ensure you have the `STARKNET_KEYSTORE` environment variable set as per [these instructions](./EnvSetUp.md#environment-variables-setup).
