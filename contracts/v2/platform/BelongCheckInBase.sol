...
import {StakingTiers, VenueRules, PaymentTypes, BountyTypes, LongPaymentTypes, VenueInfo, CustomerInfo, PromoterInfo} from "../Structures.sol";
...
abstract contract BelongCheckInBase is Initializable, Ownable {
    ...
    function updateVenueRules(VenueRules calldata rules) external {
        ...
        uint256 venueBalance = belongCheckInStorage
            .contracts
            .venueToken
            .balanceOf(msg.sender, venueId);
    ...
    function venueDeposit(VenueInfo calldata venueInfo) external {
        ...
        uint256 convenienceFeeLong = _swapUSDCtoLONG(
            address(_storage.contracts.escrow),
    ...
    function payToVenue(CustomerInfo calldata customerInfo) external {
        ...
        if (customerInfo.paymentInUSDC) {
            ...
            uint256 longFromCustomer = customerInfo.amount -
            ...
            if (rules.longPaymentType == LongPaymentTypes.AutoStake) {
                _storage.paymentsInfo.long.safeApprove(
                    address(_storage.contracts.staking),
                    longAmount
                );
                _storage.contracts.staking.deposit(
                    longAmount,
                    customerInfo.venueToPayFor
                );
                ...
                _storage.paymentsInfo.long.safeTransfer(
                    customerInfo.venueToPayFor,
                    longAmount
                );
            }
    ...
    function _swapUSDCtoLONG(
        address recipient,
        uint256 amount
    ) internal virtual returns (uint256 swapped);
